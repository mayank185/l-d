...
CREATE TABLE user_role(
    ...
    last_modified_date TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    is_active boolean DEFAULT true,
    FOREIGN KEY(user_id) REFERENCES org_user(id) /*@ManyToMany(role_enum)*/
);
...
CREATE TABLE goal_member(
    ...
    FOREIGN KEY (goal_id) REFERENCES goal(id) /*@ManyToMany(org_user)*/
);
...
CREATE TABLE goal_prerequisite(
   id BIGSERIAL NOT NULL PRIMARY KEY ,
    ...
    FOREIGN KEY (prerequisite_goal_id) REFERENCES goal(id) /*@ManyToMany(goal)*/
);
